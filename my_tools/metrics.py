...
def fast_confusion(class_num, true, pred, label_values=None):
    """
    ...
    label_values=np.array(range(class_num))
...
def fast_metrics(confusions, ignore_unclassified=False):
    """
    ...
    TP = np.diagonal(confusions, axis1=-2, axis2=-1) # 对角线
    TP_plus_FP = np.sum(confusions, axis=-1) # gt
    TP_plus_FN = np.sum(confusions, axis=-2) # pred
    ...
    # Compute Avg_acc
    mask = TP_plus_FP < 1e-6
    REC = TP / (TP_plus_FP + 1e-6)
    ACC_avg = np.mean(REC[~mask])
    ...
    # Compute Kappa
    po=ACC
    pe=np.sum(TP_plus_FN*TP_plus_FP)/(np.sum(confusions, axis=(-2, -1))**2+ 1e-6)
    ...
    mask = TP_plus_FP < 1e-3
    counts = np.sum(1 - mask, axis=-1)
    mIoU = np.sum(IoU, axis=-1) / (counts + 1e-6)

    return ACC, ACC_avg, Kappa, mIoU, IoU
