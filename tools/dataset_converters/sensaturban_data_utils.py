...
class SensatUrbanData(object):
    """SensatUrban data.

    Generate SensatUrban infos for outdoor_converter.
    ...
    def get_infos(self, has_label=True):
        ...
        """
        if not self.split.endswith(".ply"):
            path = osp.join(self.data_dir, self.split + ".ply")
        else:
            path = osp.join(self.data_dir, self.split)
        print(f"Start process {path}")
        info = dict()
        pc_info = {"num_features": 6, "lidar_idx": f"{self.split}"}
        info["point_cloud"] = pc_info
        pts_filename = osp.join(
            self.root_dir, "sensaturban_data", f"{self.split}_point.npy"
        )
        pts_semantic_mask_path = osp.join(
            self.root_dir, "sensaturban_data", f"{self.split}_sem_label.npy"
        )
        mmengine.mkdir_or_exist(osp.join(self.root_dir, "sensaturban_data"))
...
class SensatUrbanSegData(object):
    """SensatUrban dataset used to generate infos for semantic segmentation task.
    ...
    def __init__(self, data_root, ann_file, num_points, split, label_weight_func=None):
        ...
        self.cat_ids = np.array(range(13))  # used for seg task
