...
class IndoorPatchPointSample(BaseTransform):
    ...
    def __init__(
        ...
        sample_type: str = "RandomSp",
        sample_rate: Optional[float] = None,
        sample_voxel_size: Optional[float] = 0.5,
    ...
    def _patch_points_sampling(
        ...
        assert self.sample_type in ["RandomSp", "VoxelSp", "FarthestPointSp"]
        if self.sample_type == "RandomSp":
        ...
        elif self.sample_type == "VoxelSp":
            # get points of patch
            points = coords[point_idxs, :]
...
class LaserMix(BaseTransform):
    ...
    def laser_mix_transform(self, input_dict: dict, mix_results: dict) -> dict:
        ...
        pitch = torch.clamp(
            pitch, self.pitch_angles[0] + 1e-5, self.pitch_angles[1] - 1e-5
        )
        ...
        mix_pitch = torch.clamp(
            mix_pitch, self.pitch_angles[0] + 1e-5, self.pitch_angles[1] - 1e-5
        )

        num_areas = np.random.choice(self.num_areas, size=1)[0]
        angle_list = np.linspace(
            self.pitch_angles[1], self.pitch_angles[0], num_areas + 1
        )
        out_points = []
