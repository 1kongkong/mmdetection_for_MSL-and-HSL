...
class PAConv(nn.Module):
    ...
    def __init__(
        ...
        weight_bank = (
            weight_bank.permute(1, 0, 2).reshape(in_channels, self.num_kernels * out_channels).contiguous()
        )
...
class PAConvCUDA(PAConv):
    ...
    def forward(self, inputs: Tuple[Tensor]) -> Tuple[Tensor]:
        ...
        ## 适配AMP
        if point_feat.dtype == torch.float16 or center_feat.dtype == torch.float16:
            point_feat = point_feat.to(torch.float32)
            center_feat = center_feat.to(torch.float32)
            new_features = assign_score_cuda(
                scores, point_feat, center_feat, points_idx, "sum"
            ).contiguous()  # [B, out_c, npoint, K]
            new_features = new_features.to(torch.float16)
        else:
            new_features = assign_score_cuda(
                scores, point_feat, center_feat, points_idx, "sum"
            ).contiguous()  # [B, out_c, npoint, K]
