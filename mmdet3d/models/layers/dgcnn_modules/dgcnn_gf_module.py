...
class BaseDGCNNGFModule(nn.Module):
    ...
    def forward(self, points: Tensor) -> Tensor:
        ...
        for i in range(len(self.groupers)):
            ...
            if self.knn_modes[i] == "D-KNN":
                # (B, N, C) -> (B, N, K)
                idx = self.groupers[i](new_points[..., -3:].contiguous(), new_points[..., -3:].contiguous())[
                    -1
                ]

                grouped_results = grouping_operation(new_points_trans, idx)  # (B, C, N) -> (B, C, N, K)
                grouped_results -= new_points_trans.unsqueeze(-1)
            else:
                # 适配AMP
                if new_points.dtype == torch.float16:
                    new_points = new_points.to(torch.float32)
                    grouped_results = self.groupers[i](new_points, new_points)  # (B, N, C) -> (B, C, N, K)
                    grouped_results.to(torch.float16)
                else:
                    grouped_results = self.groupers[i](new_points, new_points)  # (B, N, C) -> (B, C, N, K)
